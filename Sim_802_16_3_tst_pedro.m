...
function x = bin_arr2msg(bin_arr)
    
    x = [];
    m = reshape(bin_arr,4,(numel(bin_arr)/4));
    
    for n = 1:length(m(1,:))
        x=[x bin2symbol(reshape(m(:,n),1,4))];
    end
    
end

function x = msg2bin(msg)
    
    x = [];
    
    for n = 1:length(msg)
        x=[x symbol2bin(msg(n))];
    end
    
end

% Mensagem que serah enviada
msg='ABCDEFGABCDEFGHIJLMNOPQABABABABAAAAAAAABCDEFGOABCODCABCQP';
bits = msg2bin(msg);
